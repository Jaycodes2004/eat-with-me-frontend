...
export function POSBilling() {
	...
	useEffect(() => {
		const loadTables = async () => {
			try {
				...
				addNotification({
					title: 'Error',
					message: 'Failed to load tables from server',
					type: 'error',
				});
			} finally {
	...
	const updateTable = async (tableId: string, updates: any) => {
		try {
			await updateTableAPI(tableId, updates);
			// Refresh table list
			const data = await fetchTables();
			setTables(data || []);
		} catch (error) {
	...
	const [showKitchenSuccessDialog, setShowKitchenSuccessDialog] =
		useState(false);
	const [pendingItem, setPendingItem] = useState<any>(null);
	const [billingMode, setBillingMode] = useState(false); // true when viewing a table for final billing
	...
	useEffect(() => {
		const loadCustomers = async () => {
			try {
				...
				addNotification({
					title: 'Error',
					message: 'Failed to load customers from server',
					type: 'error',
				});
			} finally {
	...
	const categories = [
		...new Set(contextMenuItems.map((item) => item.category)),
	];
	...
	const taxCalculation = cart.reduce(
		(acc, item) => {
			const menuItem = contextMenuItems.find((mi) => mi.id === item.id);
			const itemTotal = item.price * item.quantity;
			const itemTaxCategory =
				menuItem?.taxCategory || settings.defaultTaxCategory;
		...
		{
			taxes: [] as Array<{ name: string; rate: number; amount: number }>,
			totalTax: 0,
		}
	);
	...
	const handleOrderTypeSelect = (type: 'dine-in' | 'takeaway') => {
		...
		if (type === 'dine-in') {
			...
			// For takeaway, add pending item to cart immediately
			if (pendingItem) {
	...
	const handlePaymentMethodSelect = (
		...
		if (billingMode) {
			// In billing mode, complete the billing
			completeFinalBilling();
		} else {
			// In normal mode, show invoice dialog
			setShowInvoiceDialog(true);
		}
	};

	const sendToKitchen = () => {
		// Send order to kitchen as "pending"
		const newOrder = {
			id: `ORD${Date.now()}`,
			tableNumber:
				orderDetails.type === 'dine-in' && orderDetails.tableNumber
			...
			waiter: orderDetails.customerName || 'POS System',
		...
		addNotification({
			...
			message: `Order #${newOrder.id.slice(-4)} has been sent to the kitchen${
				orderDetails.tableNumber ? ` for ${orderDetails.tableNumber}` : ''
			}`,
			type: 'success',
		});
  ...
  try {
	...
	const completeFinalBilling = () => {
		...
		// ============ LOYALTY LOGIC - AWARD POINTS & HANDLE NEW CUSTOMERS ============
		if (orderDetails.customerName && orderDetails.customerPhone) {
			...
			if (existingCustomer) {
				// Existing customer - award loyalty points
				...
				// New customer - create profile with loyalty points
				...
				addNotification({
					...
					} joined loyalty program with ${Math.floor(total)} points`,
					type: 'success',
				});
			}
		}
	...
	const occupiedTables = tables
		...
		.map((table) => {
			const pendingOrders = orders.filter(
				(order) =>
					order.tableNumber === table.number && order.status === pendingStatus
			);
	...
	const downloadExcel = () => {
		const invoiceData = {
			'Invoice Details': [
				...
				[
					'Order Type',
					orderDetails.type === 'dine-in' ? 'Dine-In' : 'Takeaway',
				],
		...
		// Set column widths
		ws['!cols'] = [{ width: 25 }, { width: 10 }, { width: 15 }, { width: 15 }];
	...
	const downloadPDF = () => {
		...
		doc.text('Thank you for visiting Eat With Me!', pageWidth / 2, yPosition, {
			align: 'center',
		});
	...
	return (
		<div className='flex flex-col lg:flex-row h-full bg-background'>
			...
			<div className='flex-1 p-4 space-y-4'>
							...
							<div className='flex items-center gap-3'>
								...
								<div>
									...
									<p className='text-sm text-muted-foreground'>
										...
										{billingMode && ' • Final Billing Mode'}
									</p>
								</div>
							</div>
							<Button
				...
				{/* Occupied Tables - For Final Billing */}
				{occupiedTables.length > 0 && !billingMode && (
					<Card className='p-4'>
						<h3 className='font-semibold text-primary mb-3'>
							Occupied Tables - Click to Bill
						</h3>
						<div className='grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-5 gap-3'>
							{occupiedTables.map((table) => (
								<Card
									key={table.id}
									className='cursor-pointer hover:shadow-lg transition-all duration-200 border-2 border-amber-300 bg-amber-50 hover:bg-amber-100'
									onClick={() => loadTableForBilling(table.number)}>
									<CardContent className='p-3'>
										<div className='flex flex-col items-center text-center'>
											<Table className='w-6 h-6 text-amber-700 mb-1' />
											<p className='font-semibold text-amber-900'>
												Table {table.number}
											</p>
											<p className='text-xs text-amber-700'>{table.customer}</p>
											<Badge
												variant='outline'
												className='mt-1 text-[10px] border-amber-400 text-amber-800'>
												{table.pendingOrderCount} order
												{table.pendingOrderCount !== 1 ? 's' : ''}
											</Badge>
							...
							<div className='space-y-4 border-t border-border pt-4'>
								...
								<Button
									...
									onClick={() => {
										...
										// Show success dialog
										setShowKitchenSuccessDialog(true);

										// Clear cart but keep order details for adding more items to same table
										setCart([]);
									}}>
									<Users
										className='mr-2'
			...
			<Dialog
				...
				onOpenChange={setShowCustomerDetailsDialog}>
				<DialogContent>
					<DialogHeader>
								...
								<div className='grid grid-cols-3 gap-2 max-h-40 overflow-y-auto'>
									{availableTables.map((table) => (
										<Button
											key={table.id}
											variant={
												orderDetails.tableNumber === `Table ${table.number}`
													? 'default'
													: 'outline'
											}
							...
							!customers.find(
								...
								<div className='space-y-2'>
									...
									<p className='text-xs text-muted-foreground'>
										New customer? Enter a referral code to get 100 bonus loyalty
										points!
									</p>
								</div>
							)}
			...
			<Dialog
				...
				<DialogContent>
					<DialogHeader>
						<DialogTitle>
							Order Complete - {settings.currencySymbol}
							{total.toFixed(2)}
						</DialogTitle>
						<DialogDescription>
			...
			<Dialog
				open={showKitchenSuccessDialog}
				onOpenChange={setShowKitchenSuccessDialog}>
				<DialogContent>
					<DialogHeader>
						<DialogTitle>Order Sent to Kitchen!</DialogTitle>
						<DialogDescription>
							Your order has been sent to the kitchen. Customers can add more
							items before final checkout.
						</DialogDescription>
					</DialogHeader>
					<div className='space-y-4 py-4'>
						<div className='bg-green-50 p-4 rounded-lg border border-green-200'>
							<p className='text-green-800 font-medium'>Order Sent!</p>
							<p className='text-sm text-green-600'>
								Order #
								<span className='font-bold'>
									{orderDetails.tableNumber
										? `Table ${orderDetails.tableNumber.replace('Table ', '')}`
										: 'Takeaway'}
								</span>{' '}
						...
						<div className='space-y-2'>
							<Button
								className='w-full h-12 gap-2'
								onClick={() => setShowKitchenSuccessDialog(false)}>
								<Users
									className='mr-2'
									size={18}
								/>
