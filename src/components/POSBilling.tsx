...
export function POSBilling() {
  ...
  const [showKitchenSuccessDialog, setShowKitchenSuccessDialog] = useState(false);
  ...
  const sendToKitchen = () => {
    ...
    addNotification({
      ...
      message: `Order #${newOrder.id.slice(-4)} has been sent to the kitchen${orderDetails.tableNumber ? ` for ${orderDetails.tableNumber}` : ''}`,
      type: 'success'
    });
  ...
  const loadTableForBilling = (tableNumber: number) => {
    ...
    // Set order details from the first order
    setOrderDetails({
      type: 'dine-in',
      tableNumber: `Table ${tableNumber}`,
      customerName: tablePendingOrders[0].customerName || '',
      customerPhone: tablePendingOrders[0].customerPhone || '',
      paymentMethod: null,
      referralCode: '',
      specialInstructions: ''
    });
    ...
    addNotification({
      ...
      message: `Loaded ${tablePendingOrders.length} pending order(s) from Table ${tableNumber}`,
      type: 'info'
    });
  };
  ...
  const completeFinalBilling = () => {
    ...
    const tableNumber = orderDetails.tableNumber ? parseInt(orderDetails.tableNumber.replace('Table ', '')) : undefined;
    ...
    if (orderDetails.customerName && orderDetails.customerPhone) {
      ...
      if (existingCustomer) {
        ...
        addNotification({
          ...
          message: `${orderDetails.customerName} joined loyalty program with ${Math.floor(total)} points`,
          type: 'success'
        });
      }
    }
    ...
    <div className="flex flex-col lg:flex-row h-full bg-background">
      ...
      <div className="flex-1 p-4 space-y-4">
          ...
          <Card className="p-4 bg-primary/5 border-primary/20">
            <div className="flex items-center justify-between">
              ...
              <Button 
                ...
                size="sm"
                onClick={() => {
                  setOrderDetails({ type: null, customerName: '', customerPhone: '', paymentMethod: null, referralCode: '', specialInstructions: '' });
                  setCart([]);
                  setBillingMode(false);
                }}
              >
        ...
        {occupiedTables.length > 0 && !billingMode && (
          <Card className="p-4">
            <h3 className="font-semibold text-primary mb-3">Occupied Tables - Click to Bill</h3>
                ...
                >
                  <CardContent className="p-3">
                    <div className="flex flex-col items-center text-center">
                      ...
                      <p className="font-semibold text-amber-900">Table {table.number}</p>
                      <p className="text-xs text-amber-700">{table.customer}</p>
                      <Badge variant="outline" className="mt-1 text-[10px] border-amber-400 text-amber-800">
                        {table.pendingOrderCount} order{table.pendingOrderCount !== 1 ? 's' : ''}
                      </Badge>
                      <p className="text-sm font-medium text-amber-900 mt-1">
                        {settings.currencySymbol}{table.totalAmount.toFixed(2)}
                      </p>
                    </div>
                  </CardContent>
                </Card>
          ...
          <div className="space-y-4 py-4">
            ...
            <div className="flex gap-2 pt-4">
              ...
              <Button
                ...
                className="flex-1"
                disabled={orderDetails.type === 'dine-in' && !orderDetails.tableNumber}
              >
                Continue
              </Button>
            </div>
      ...
      <Dialog open={showKitchenSuccessDialog} onOpenChange={setShowKitchenSuccessDialog}>
        <DialogContent>
          <DialogHeader>
            <DialogTitle>Order Sent to Kitchen!</DialogTitle>
            <DialogDescription>
              Your order has been sent to the kitchen. Customers can add more items before final checkout.
            </DialogDescription>
          </DialogHeader>
          <div className="space-y-4 py-4">
            <div className="bg-green-50 p-4 rounded-lg border border-green-200">
              ...
              <p className="text-sm text-green-600">
                Order #<span className="font-bold">{orderDetails.tableNumber ? `Table ${orderDetails.tableNumber.replace('Table ', '')}` : 'Takeaway'}</span> has been sent to the kitchen.
              </p>
            </div>
