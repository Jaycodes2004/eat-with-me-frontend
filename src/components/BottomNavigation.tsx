...
export function BottomNavigation({
	...
	const navItems = bottomNavItems
		.map((item) => allNavItems[item as keyof typeof allNavItems])
		.filter(Boolean);
	...
	const groupedModules = allModules.reduce(
	...
	return (
		// FIXED: Changed from fixed positioning to a proper bottom layout
		<div className='fixed bottom-0 left-0 right-0 bg-card border-t border-border shadow-2xl z-50'>
			{/* FIXED: Removed max-w-md constraint and adjusted padding for better spacing */}
			<div className='flex items-center justify-around py-3 px-2 w-full'>
				{navItems.map((item) => {
					...
					<SheetContent
						...
						<div className='space-y-6 pb-20'>
							{Object.entries(groupedModules).map(([category, modules]) => (
								<div key={category}>
									<h3 className='mb-3 px-2 text-muted-foreground'>
										{category}
									</h3>
