...
export function TableManagement({ onNavigate }: TableManagementProps) {
  ...
  const handleTableAction = (
    table: Table,
    action: 'start-order' | 'resume-order' | 'checkout',
  ) => {
    if (action === 'start-order' || action === 'resume-order') {
      // Set the selected table in global context and navigate to POS
      setSelectedTable(table.id);
  ...
  const handleCheckout = async (action: 'print' | 'whatsapp' | 'save') => {
    ...
    if (action === 'print') {
      ...
      if (customerPhone) {
        const message = `Hi ${
          checkoutTable.customer ?? 'guest'
        }, your bill total is ${safeCurrency(
          total,
          currencySymbol,
    ...
    <div className="p-4 space-y-6">
      ...
      <div className="flex flex-col sm:flex-row gap-4 items-start sm:items-center justify-between">
        ...
        <div className="flex gap-2">
          <Select
            value={viewMode}
            onValueChange={(value: 'compact' | 'detailed') => setViewMode(value)}
          >
            <SelectTrigger className="w-32">
              <SelectValue />
            </SelectTrigger>
      ...
      <div className="flex flex-col sm:flex-row gap-4">
        ...
        <Select
          value={statusFilter}
          onValueChange={(value: TableStatus | 'all') => setStatusFilter(value)}
        >
          <SelectTrigger className="w-full sm:w-48">
            <SelectValue placeholder="Filter by status" />
          </SelectTrigger>
        ...
        <Card className="p-4">
          <div className="text-center">
            <div className="text-2xl font-bold text-primary">
              {safeCurrency(stats.revenue, currencySymbol)}
            </div>
            <div className="text-sm text-muted-foreground">Revenue</div>
          </div>
        </Card>
                ...
                <div className="space-y-4">
                  <div className="grid grid-cols-2 gap-4">
                    ...
                    <div>
                      ...
                      <Input
                        id="location"
                        value={table.location ?? 'Not specified'}
                        readOnly
                      />
                    </div>
                  </div>
              ...
              <div className="bg-primary/5 p-4 rounded-lg">
                ...
                <div className="space-y-1 text-sm">
                  ...
                  <div className="flex justify-between">
                    <span>Discount ({discount}%):</span>
                    <span>
                      -{safeCurrency(checkoutDiscountAmount, currencySymbol)}
                    </span>
                  </div>
                  <div className="flex justify-between">
                    <span>Subtotal:</span>
