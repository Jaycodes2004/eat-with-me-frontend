...
export function Settings() {
	...
	const handleAddTaxRule = async () => {
		if (
			!newTaxRule.name ||
			!newTaxRule.rate ||
			!newTaxRule.applicableCategories?.length
		) {
	...
	const [userRoles] = useState([
		{
			id: '1',
			name: 'Admin',
			email: `admin@${settings.restaurantName
				.toLowerCase()
				.replace(/\s+/g, '')}.com`,
			role: 'Administrator',
		},
		...
		},
		{
			id: '3',
			name: 'Cashier',
			email: `cashier@${settings.restaurantName
				.toLowerCase()
				.replace(/\s+/g, '')}.com`,
			role: 'Cashier',
		},
		{
			id: '4',
			name: 'Waiter',
			email: `waiter@${settings.restaurantName
				.toLowerCase()
				.replace(/\s+/g, '')}.com`,
			role: 'Staff',
		},
	...
	const handleCountryChange = (country: string) => {
		const countryConfig = countryCurrencyMap[country];
		updateSettings({
			country,
	...
	return (
		<div className='flex-1 bg-background p-3 sm:p-4 md:p-6 space-y-4 sm:space-y-6 animate-slide-up pb-24'>
			...
			<div className='flex flex-col sm:flex-row items-start sm:items-center justify-between gap-3 sm:gap-0'>
				...
				<Button
					onClick={handleSave}
					className={`bg-primary hover:bg-primary/90 transition-all duration-200 w-full sm:w-auto ${
						saved ? 'bg-green-600' : ''
					}`}>
					{saved ? (
						<Check
			...
			<Tabs
							...
							<div className='flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3'>
								...
								<div className='flex items-center gap-2'>
									...
									<Button
										...
										disabled={!businessInfo.restaurantId}>
								...
								<div className='space-y-2'>
									<Label>
										{settings.country === 'India'
											? 'GST Number'
											: ['United Kingdom', 'Germany', 'France'].includes(
													settings.country
											  )
											? 'VAT Number'
											: 'Tax Number'}
							...
							<div className='flex items-center justify-between p-4 bg-muted/30 rounded-lg'>
								<div className='flex-1'>
									...
									<p className='text-sm text-muted-foreground mt-1'>
										Enable or disable the loyalty and referral program for
										customers
									</p>
								</div>
								<Switch
									...
									onCheckedChange={(checked: boolean) =>
										updateSettings({ loyaltyEnabled: checked })
									}
								/>
							</div>
							...
							{settings.loyaltyEnabled && (
								<div className='p-3 sm:p-4 bg-gradient-to-br from-primary/5 to-primary/10 rounded-lg border border-primary/20'>
									...
									<p className='text-xs sm:text-sm text-muted-foreground mb-4'>
										Set how many loyalty points customers earn for every{' '}
										{settings.currencySymbol}1 spent
									</p>
									<div className='flex flex-col sm:flex-row items-start sm:items-center gap-4'>
										<div className='w-full sm:flex-1'>
											<Label className='text-xs sm:text-sm mb-2'>
												Points earned per {settings.currencySymbol}1
											</Label>
										...
										<div className='w-full sm:flex-1 p-3 bg-card rounded-lg border'>
											...
											<p className='text-xs sm:text-sm'>
												{settings.currencySymbol}100 purchase ={' '}
												<span className='font-semibold text-primary'>
													{(100 * settings.loyaltyPointsPerCurrency).toFixed(1)}{' '}
													points
												</span>
											</p>
										</div>
								...
								<Dialog
									...
									<DialogContent>
										<DialogHeader>
											...
											<DialogDescription>
												Create custom tax rules for different categories of
												items
											</DialogDescription>
										</DialogHeader>
													...
													{taxCategories.map((category) => (
														<button
															...
															className={`px-3 py-2 rounded-lg text-sm transition-colors ${
																newTaxRule.applicableCategories?.includes(
																	category.value
																)
							...
							<div className='grid gap-4'>
													...
													<div className='flex flex-wrap gap-1'>
														{tax.applicableCategories.map((cat) => (
															<Badge
																...
																variant='outline'
																className='text-xs'>
																{taxCategories.find((c) => c.value === cat)
																	?.label || cat}
															</Badge>
														))}
													</div>
								...
								{settings.taxRules.length === 0 && (
									<div className='text-center py-8 text-muted-foreground'>
										...
										<p className='text-sm'>
											Add your first tax rule to get started
										</p>
									</div>
								)}
							</div>
							...
							<Card className='bg-blue-50 border-blue-200'>
								<CardContent className='p-4'>
									...
									<p className='text-xs text-blue-600 mt-2'>
										* Preview shows taxes for food items
									</p>
								</CardContent>
							</Card>
						</CardContent>
				...
				{/* Staff redirect tab (if still needed) */}
				<TabsContent
					value='staff-redirect'
					className='space-y-6'>
					<Card className='border-0 shadow-lg'>
						<CardContent className='p-6 text-center'>
							...
							<p className='text-muted-foreground mb-4'>
								All staff management, roles, and permissions have been moved to
								the dedicated Staff Management module.
							</p>
							<Button
								...
								<div className='space-y-4'>
									...
									<div className='space-y-2'>
										...
										<Select
											...
											<SelectContent>
												<SelectItem value='Receipt Printer 1'>
													Receipt Printer 1
												</SelectItem>
												<SelectItem value='Receipt Printer 2'>
													Receipt Printer 2
												</SelectItem>
												<SelectItem value='Disabled'>Disabled</SelectItem>
											</SelectContent>
									...
									<div className='flex flex-col sm:flex-row items-start sm:items-center justify-between p-3 bg-white rounded-lg gap-3'>
										<div className='flex-1'>
											...
											<p className='text-xs sm:text-sm text-muted-foreground'>
												Send order confirmations and promotions
											</p>
										</div>
										<Switch
									...
									{whatsappSettings.apiKey && whatsappSettings.phoneNumber && (
										<div className='p-3 bg-green-100 border border-green-300 rounded-lg'>
											<p className='text-sm text-green-800'>
												✅ WhatsApp API is configured and ready for marketing
												campaigns
											</p>
										</div>
									)}
								...
								<CardContent className='space-y-4'>
									...
									<div className='flex flex-col sm:flex-row items-start sm:items-center justify-between p-3 bg-white rounded-lg gap-3'>
										<div className='flex-1'>
											...
											<p className='text-xs sm:text-sm text-muted-foreground'>
												Automatically accept Zomato orders
											</p>
										</div>
								...
								<CardContent className='space-y-4'>
									...
									<div className='flex flex-col sm:flex-row items-start sm:items-center justify-between p-3 bg-white rounded-lg gap-3'>
										<div className='flex-1'>
											...
											<p className='text-xs sm:text-sm text-muted-foreground'>
												Automatically accept Swiggy orders
											</p>
										</div>
										<Switch className='self-end sm:self-auto' />
									</div>
							...
							<Card className='border-blue-200 bg-blue-50'>
								<CardHeader className='pb-3'>
									...
									<p className='text-sm text-blue-700 mt-1'>
										Configure payment processing for QR orders, online orders,
									...
									<div className='space-y-4'>
										...
										<div className='grid grid-cols-1 md:grid-cols-2 gap-4'>
											<div className='space-y-2'>
												...
												<Input
													placeholder='Enter secret/private key'
													type='password'
												/>
											</div>
											<div className='space-y-2'>
