...
export function LoginScreen({ onLogin }: LoginScreenProps) {
	...
	const handleLogin = async () => {
		if (!email || !password || !restaurantId) {
			const message =
				'Please enter your restaurant ID, email, and password to continue.';
			setError(message);
			toast.error(message);
			return;
		}
		...
		try {
			...
			try {
				localStorage.setItem(
					'allowedModules',
					JSON.stringify(data.restaurant.allowedModules || [])
				);
			} catch (e) {
			...
			const message =
				...
				'Failed to sign in. Please verify your credentials and restaurant ID.';
			setError(message);
			toast.error(message);
		} finally {
			setIsLoading(false);
	...
	const handleSignupSuccess = (payload: SignupSuccessPayload) => {
		...
		toast.success(
			`Restaurant created successfully! Note your Restaurant ID: ${payload.restaurantId}`
		);
	};
