...
export type InventoryWastageReason =
	| 'expired'
	| 'damaged'
	| 'overcooked'
	| 'customer-return'
...
interface AppContextType {
	...
	recordInventoryWastage: (
		payload: CreateInventoryWastageRecordPayload
	...
	getRevenueByPaymentMethod: (dateFilter?: string) => {
		cash: number;
		digital: number;
		split: number;
	};
	...
	updateCustomerStats: (
		customerId: string,
		newOrder: { amount: number; date: string; items: string[] }
	) => void;
	...
	updateReservation: (
		id: string,
		updates: UpdateReservationPayload
	) => Promise<Reservation>;
	...
	addSalaryPayment: (
		payment: CreateSalaryPaymentPayload
...
const defaultRecipes: Recipe[] = [
	{
		...
		ingredients: [
			...
			{
				inventoryItemId: '5',
				inventoryItemName: 'Yogurt',
				quantity: 0.1,
				unit: 'kg',
			},
			{
				inventoryItemId: '3',
				inventoryItemName: 'Spices',
				quantity: 0.05,
				unit: 'kg',
			},
		],
...
const generateDefaultOrders = (): Order[] => {
	const orderSources: Order['orderSource'][] = [
		'dine-in',
		'zomato',
		'swiggy',
		'takeaway',
	...
	return [
		...
		},
		{
			...
			status: 'ready',
			orderTime: new Date(Date.now() - 30 * 60 * 1000).toLocaleTimeString(
				'en-US',
				{ hour: '2-digit', minute: '2-digit', hour12: true }
			),
			orderDate: new Date().toISOString().split('T')[0],
		...
		},
		{
			...
			subtotal: 440,
			taxes: [],
			completedAt: new Date(Date.now() - 30 * 60 * 1000).toLocaleTimeString(
				'en-US',
				{ hour: '2-digit', minute: '2-digit', hour12: true }
			),
			rating: 4.5,
		...
		// Additional completed orders with ratings for AI analysis
		{
			...
			subtotal: 400,
			taxes: [],
			completedAt: new Date(Date.now() - 90 * 60 * 1000).toLocaleTimeString(
				'en-US',
				{ hour: '2-digit', minute: '2-digit', hour12: true }
			),
			rating: 5,
		...
		},
		{
			...
			status: 'completed',
			orderTime: new Date(Date.now() - 3 * 60 * 60 * 1000).toLocaleTimeString(
				'en-US',
				{ hour: '2-digit', minute: '2-digit', hour12: true }
			),
			orderDate: new Date().toISOString().split('T')[0],
			...
			subtotal: 330,
			taxes: [],
			completedAt: new Date(Date.now() - 150 * 60 * 1000).toLocaleTimeString(
				'en-US',
				{ hour: '2-digit', minute: '2-digit', hour12: true }
			),
			rating: 4,
		...
		},
		{
			...
			status: 'completed',
			orderTime: new Date(Date.now() - 4 * 60 * 60 * 1000).toLocaleTimeString(
				'en-US',
				{ hour: '2-digit', minute: '2-digit', hour12: true }
			),
			orderDate: new Date().toISOString().split('T')[0],
			...
			subtotal: 310,
			taxes: [],
			completedAt: new Date(Date.now() - 210 * 60 * 1000).toLocaleTimeString(
				'en-US',
				{ hour: '2-digit', minute: '2-digit', hour12: true }
			),
			rating: 3.5,
		...
		},
		{
			...
			status: 'completed',
			orderTime: new Date(Date.now() - 6 * 60 * 60 * 1000).toLocaleTimeString(
				'en-US',
				{ hour: '2-digit', minute: '2-digit', hour12: true }
			),
			orderDate: new Date().toISOString().split('T')[0],
			...
			subtotal: 740,
			taxes: [],
			completedAt: new Date(Date.now() - 330 * 60 * 1000).toLocaleTimeString(
				'en-US',
				{ hour: '2-digit', minute: '2-digit', hour12: true }
			),
			rating: 4.8,
		...
		},
		{
			...
			status: 'completed',
			orderTime: new Date(Date.now() - 8 * 60 * 60 * 1000).toLocaleTimeString(
				'en-US',
				{ hour: '2-digit', minute: '2-digit', hour12: true }
			),
			orderDate: new Date().toISOString().split('T')[0],
			...
			subtotal: 280,
			taxes: [],
			completedAt: new Date(Date.now() - 450 * 60 * 1000).toLocaleTimeString(
				'en-US',
				{ hour: '2-digit', minute: '2-digit', hour12: true }
			),
			rating: 4.2,
...
export const AppProvider: React.FC<{ children: ReactNode }> = ({
	...
	const [inventoryWastageRecords, setInventoryWastageRecords] = useState<
		InventoryWastageRecord[]
	>(defaultInventoryWastageRecords);
	...
	const [loyaltyRules, setLoyaltyRules] =
		useState<LoyaltyRule[]>(defaultLoyaltyRules);

	// New data state
	const [recipes, setRecipes] = useState<Recipe[]>(defaultRecipes);
	const [extendedCustomers, setExtendedCustomers] = useState<
		ExtendedCustomer[]
	>(defaultExtendedCustomers);
	const [expenses, setExpenses] = useState<Expense[]>(defaultExpenses);
	const [reservations, setReservations] =
		useState<Reservation[]>(defaultReservations);
	...
	const bottomNavigation = currentUser
		? getBottomNavigationForRole(currentUser.role)
		: [];
	const quickActions = currentUser
		? getQuickActionsForRole(currentUser.role)
		: [];
	...
	const normalizeReservationPriority = (
		value: unknown
	...
	const normalizeOrderSource = (value: unknown): Order['orderSource'] => {
		const allowed: Order['orderSource'][] = [
			'dine-in',
			'zomato',
			'swiggy',
			'own-app',
	...
	const normalizeDeliveryType = (
		value: unknown
	): NonNullable<Order['deliveryType']> => {
		const allowed: NonNullable<Order['deliveryType']>[] = [
			'dine-in',
			'delivery',
			'pickup',
			'takeaway',
	...
	const normalizePaymentMethod = (
		...
		const allowed: NonNullable<Order['paymentMethod']>[] = [
			'cash',
			'card',
			'upi',
			'split',
	...
	const normalizeSalaryPaymentType = (
		value: unknown
	): SalaryPayment['paymentType'] => {
		const allowed: SalaryPayment['paymentType'][] = [
			'Full Salary',
			'Partial Payment',
			'Advance',
			'Bonus',
	...
	const normalizeSalaryPaymentStatus = (
		value: unknown
	...
	const normalizePaymentRecordStatus = (
		value: unknown
	...
	const mapSettingsPatchToApi = useCallback(
		(patch: Partial<AppSettings>): Record<string, unknown> => {
			...
			if (
				Object.prototype.hasOwnProperty.call(patch, 'loyaltyPointsPerCurrency')
			) {
	...
	const mapMenuItemToApi = useCallback(
		(item: Partial<CreateMenuItemPayload>): Record<string, unknown> => {
			const payload: Record<string, unknown> = {};
			...
			if (Object.prototype.hasOwnProperty.call(item, 'spiceLevel')) {
				...
				if (typeof spiceLevel === 'string') {
					const spiceMapping: Record<MenuItem['spiceLevel'], number> = {
						mild: 1,
						medium: 2,
						hot: 3,
					};
	...
	const mapCategoryFromApi = useCallback(
		(category: any): Category => {
			const rawType =
				typeof category?.type === 'string'
					? category.type.toLowerCase()
					: 'menu';
			const allowed: Category['type'][] = [
				'menu',
				'expense',
				'inventory',
				'supplier',
			];
			const type = allowed.includes(rawType as Category['type'])
				? (rawType as Category['type'])
				: 'menu';
			...
			return {
				...
				createdAt: category?.createdAt
					? parseIsoString(category.createdAt)
					: undefined,
				isActive:
	...
	const mapCategoryToApi = useCallback(
	...
	const mapRoleFromApi = useCallback(
		(role: any): Role => {
				...
				? Array.from(
						new Set<string>(
							role.dashboardModules
								.filter(
									(module: unknown): module is string =>
			...
			return {
				...
				staffCount:
					typeof role?.staffCount === 'number' ? role.staffCount : undefined,
	...
	const mapRoleToApi = useCallback(
		(payload: CreateRolePayload | UpdateRolePayload) => {
			...
			if (
				...
				const permissionArray = Array.isArray(payload.permissions)
					? payload.permissions
					: [];
				body.permissions = normalizePermissionList(permissionArray);
			}
			if (
				Object.prototype.hasOwnProperty.call(payload, 'dashboardModules') &&
				payload.dashboardModules !== undefined
			) {
				body.dashboardModules = Array.isArray(payload.dashboardModules)
	...
	const mapTableFromApi = useCallback(
		(table: any): Table => ({
			id: table?.id ?? '',
			...
			orderAmount:
				typeof table?.orderAmount === 'number' ? table.orderAmount : undefined,
			...
			lastOrderAt: table?.lastOrderAt
				? parseIsoString(table.lastOrderAt)
				: null,
	...
	const mapReservationFromApi = useCallback(
		(reservation: any): Reservation => ({
			...
			tableNumber:
				reservation?.tableNumber ?? reservation?.table?.number ?? undefined,
			...
			source:
				typeof reservation?.source === 'string'
					? (reservation.source.toLowerCase() as Reservation['source'])
					: 'phone',
			prepayment:
				typeof reservation?.prepayment === 'number'
	...
	const mapReservationToApi = useCallback(
		(
			reservation: Partial<CreateReservationPayload>
			...
			if (Object.prototype.hasOwnProperty.call(reservation, 'date')) {
				...
				if (dateValue) {
					...
					payload.date = Number.isNaN(parsed.getTime())
						? null
						: parsed.toISOString();
				} else {
					payload.date = null;
			...
			if (Object.prototype.hasOwnProperty.call(reservation, 'partySize')) {
				payload.partySize =
					reservation.partySize != null ? Number(reservation.partySize) : null;
			}
			...
			if (
				Object.prototype.hasOwnProperty.call(reservation, 'specialRequests')
			) {
	...
	const mapOrderFromApi = useCallback(
		(order: any): Order => {
			const orderTimeIso =
				parseIsoString(order?.orderTime) || new Date().toISOString();
			const orderDate =
				getDateOnly(orderTimeIso) || new Date().toISOString().split('T')[0];

			const estimatedTimeValue = order?.estimatedTime;
			const estimatedTime =
				typeof estimatedTimeValue === 'number'
					? estimatedTimeValue
					: (() => {
			...
			return {
				...
				items: Array.isArray(order?.items)
					? order.items.map(mapOrderItemFromApi)
					: [],
				...
				estimatedTime,
				actualCookingTime:
					typeof order?.actualCookingTime === 'number'
						? order.actualCookingTime
						: undefined,
				...
				completedAt: order?.completedAt
					? parseIsoString(order.completedAt)
					: undefined,
	...
	const mapPaymentRecordFromApi = useCallback(
		(payment: any): PaymentRecord => {
			const paymentDateIso =
				parseIsoString(payment?.paymentDate) || new Date().toISOString();
			...
			return {
				...
				deductions:
					typeof payment?.deductions === 'number'
						? payment.deductions
						: undefined,
				type,
	...
	const mapStaffFromApi = useCallback(
		(staffMember: any): Staff => {
			...
			return {
				...
				isActive:
					typeof staffMember?.isActive === 'boolean'
						? staffMember.isActive
						: true,
				joinDate: staffMember?.joinDate
					? getDateOnly(staffMember.joinDate)
					: undefined,
				salary:
					staffMember?.salary != null ? Number(staffMember.salary) : undefined,
				currentShift:
					typeof staffMember?.currentShift === 'string'
				...
				salaryDetails: {
					baseSalary: Number(
						salaryDetails?.baseSalary ?? staffMember?.salary ?? 0
					),
	...
	const mapShiftFromApi = useCallback(
		(shift: any): Shift => {
	...
	const mapSalaryPaymentFromApi = useCallback(
		(payment: any): SalaryPayment => {
			const paymentDateIso =
				parseIsoString(payment?.paymentDate) || new Date().toISOString();
			const derivedDate = new Date(paymentDateIso);
			const month =
			...
			try {
				...
				setDataError(
					failedResources.length
						? `Failed to load ${failedResources.join(', ')}`
						: null
				);
	...
	const addMenuItem = useCallback(
		async (payload: CreateMenuItemPayload): Promise<MenuItem> => {
	...
	const updateMenuItem = useCallback(
		async (id: string, updates: UpdateMenuItemPayload): Promise<MenuItem> => {
			try {
				const response = await apiClient.put(
					`/menu/${id}`,
					mapMenuItemToApi(updates)
				);
	...
	const updateStaff = useCallback(
		async (id: string, updates: UpdateStaffPayload): Promise<Staff> => {
			try {
				const body: any = {
					...updates,
				...
				const staffMember = mapStaffFromApi(response.data);
				setStaff((prev) =>
					prev.map((existing) => (existing.id === id ? staffMember : existing))
				);
				return staffMember;
			} catch (error) {
	...
	const deleteStaff = useCallback(async (id: string): Promise<void> => {
		try {
			...
			setSalaryPayments((prev) =>
				prev.filter((payment) => payment.staffId !== id)
			);
		} catch (error) {
	...
	const addSalaryPayment = useCallback(
		async (payload: CreateSalaryPaymentPayload): Promise<SalaryPayment> => {
			try {
				...
				setStaff((prev) =>
					prev.map((staffMember) => {
						...
						const existingHistory = Array.isArray(staffMember.paymentHistory)
						...
						const nextHistory =
							historyIndex >= 0
								? existingHistory.map((record) =>
										record.id === paymentRecord.id ? paymentRecord : record
								  )
	...
	const addSupplier = async (
		...
		const res = await apiClient.post('/supplier', payload);
		const created: Supplier = {
			...
			lastOrderDate:
				res.data.lastOrderDate ?? new Date().toISOString().split('T')[0],
		};
	...
	const updateSupplier = async (
		...
		const res = await apiClient.put(`/supplier/${id}`, updates);
...
const deleteCustomer = async (id: string): Promise<void> => {
  ...
  try {
		...
		async (payload: CreateCategoryPayload): Promise<Category> => {
			...
			try {
				...
				setCategories((prev) => {
					...
					return exists
						? prev.map((category) =>
								category.id === created.id ? created : category
						  )
						: [...prev, created];
				});
				return created;
	...
	const updateCategory = useCallback(
		async (id: string, updates: UpdateCategoryPayload): Promise<Category> => {
			if (!id) {
		...
		async (payload: CreateRolePayload): Promise<Role> => {
			...
			try {
				...
				setRoles((prev) => {
					...
					return exists
						? prev.map((role) =>
								role.id === createdRole.id ? createdRole : role
						  )
						: [...prev, createdRole];
				});
	...
	const updateRole = useCallback(
		async (id: string, updates: UpdateRolePayload): Promise<Role> => {
			if (!id) {
			...
			try {
				const response = await apiClient.put(
					`/category-role/roles/${id}`,
					body
				);
				const updatedFromApi = mapRoleFromApi(response.data);
				let resolvedRole: Role = {
					...updatedFromApi,
					permissions: Array.isArray(updatedFromApi.permissions)
						? updatedFromApi.permissions
						: [],
					dashboardModules: Array.isArray(updatedFromApi.dashboardModules)
						? updatedFromApi.dashboardModules
						: [],
				};

				if (updates.permissions !== undefined) {
					resolvedRole.permissions = Array.isArray(updates.permissions)
						? updates.permissions
						: [];
				}

				if (updates.dashboardModules !== undefined) {
					resolvedRole.dashboardModules = Array.isArray(
						updates.dashboardModules
					)
						? updates.dashboardModules
						: [];
				}
	...
	const recordInventoryWastage = useCallback(
		(payload: CreateInventoryWastageRecordPayload): InventoryWastageRecord => {
	...
	const addTaxRule = useCallback(
		async (taxRule: TaxRule): Promise<TaxRule> => {
			const targetRule = taxRule.id
				? taxRule
				: { ...taxRule, id: `tax_${Date.now()}` };
			const existsIndex = settings.taxRules.findIndex(
				(rule) => rule.id === targetRule.id
			);
			const updatedRules =
				existsIndex >= 0
					? settings.taxRules.map((rule) =>
							rule.id === targetRule.id ? targetRule : rule
					  )
	...
	const updateTaxRule = useCallback(
		async (id: string, updates: Partial<TaxRule>): Promise<TaxRule> => {
			...
			const updatedRules = settings.taxRules.map((rule) =>
				rule.id === id ? mergedRule : rule
			);
	...
	const deleteTaxRule = useCallback(
	...
	const calculateTaxes = (baseAmount: number, taxCategory: string) => {
		const applicableTaxes = settings.taxRules.filter(
			(rule) =>
				rule.isActive &&
	...
	const addTable = useCallback(
		async (payload: CreateTablePayload): Promise<Table> => {
			try {
	...
	const updateTable = useCallback(
		async (id: string, updates: UpdateTablePayload): Promise<Table> => {
			try {
				const body: Record<string, unknown> = {
					...
					guests:
						typeof updates.guests === 'number' ? updates.guests : undefined,
	...
	const getTodayRevenue = () => {
		...
		return orders
			.filter(
				(order) => order.orderDate === today && order.status === 'completed'
			)
		...
		switch (filter) {
			...
			case 'week':
				const weekAgo = new Date(Date.now() - 7 * 24 * 60 * 60 * 1000)
					...
					.split('T')[0];
				return orders.filter(
					(order) => order.orderDate >= weekAgo && order.orderDate <= today
				);
			case 'month':
				const monthAgo = new Date(Date.now() - 30 * 24 * 60 * 60 * 1000)
					...
					.split('T')[0];
				return orders.filter(
					(order) => order.orderDate >= monthAgo && order.orderDate <= today
				);
			case 'all':
			default:
				return orders;
	...
	const getOrderStatsByDateFilter = (
		filter: 'today' | 'yesterday' | 'week' | 'month' | 'all'
	) => {
	...
	const updateExtendedCustomer = (
		id: string,
		updates: Partial<ExtendedCustomer>
	) => {
		setExtendedCustomers((prev) =>
			prev.map((customer) =>
		...
		setExtendedCustomers((prev) =>
			prev.map((customer) => {
				if (customer.id === customerId) {
					...
					const newAverageOrderValue = Math.round(
						newTotalSpent / newVisitCount
					);
					...
					return {
						...
						loyaltyPoints:
							(customer.loyaltyPoints || 0) + Math.floor(newOrder.amount / 10),
					};
				}
				return customer;
			})
...
const deletePurchaseOrder = async (id: string): Promise<void> => {
  ...
  const loadPurchaseOrders = useCallback(async () => {
		try {
			...
			console.error('[AppContext] Failed to load purchases', error);
		}
	}, []);
	...
	const addReservation = useCallback(
		async (payload: CreateReservationPayload): Promise<Reservation> => {
			if (
			...
			try {
				...
				setReservations((prev) => {
					const exists = prev.some(
						(reservation) => reservation.id === merged.id
					);
					return exists
						? prev.map((reservation) =>
								reservation.id === merged.id ? merged : reservation
						  )
						: [...prev, merged];
				});
	...
	const syncCustomerFromTable = (table: Table) => {
		...
		if (table.status === 'occupied' && table.customer) {
			...
			const existingCustomer = extendedCustomers.find(
				(c) => c.name.toLowerCase() === table.customer?.toLowerCase()
			);
	...
	const awardLoyaltyPoints = (customerId: string, orderAmount: number) => {
		...
		const activeRules = loyaltyRules.filter(
			(rule) => rule.isActive && rule.type === 'earn'
		);
		...
		const bonusRules = loyaltyRules.filter(
			(rule) =>
				rule.isActive &&
				rule.type === 'bonus' &&
				rule.minOrderValue &&
